		...
		<br>
			...
			<div class="tab-pane box" id="add" style="padding: 5px">
				<div class="box-content">
					...
					<div class="form-group">
						<label class="col-md-3 control-label">Date of Burial <span class="required">*</span></label>
						<div class="col-md-7">
							<div class="input-daterange input-group" data-plugin-datepicker>
								<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
...
$(document).ready(function() {
				...
				if (response.success && response.members.length > 0) {
					...
					$.each(response.members, function(index, member) {
						...
						resultsHtml += '<div class="member-search-result" style="padding: 10px 15px; border-bottom: 1px solid #f0f0f0; cursor: pointer;" ' +
							...
							'data-member-status="' + (member.status || '') + '">' +  <!-- Added status -->
					...
					$('.member-search-result').click(function() {
						var member = {
							...
							status: $(this).data('member-status')   // ← New
						};
						...
						// === STATUS CHECK - TURN RED IF STATUS IS 0 ===
						var $memberInfo = $('#member_info');
						if (member.status == 0 || member.status === '0') {
							$memberInfo.removeClass('alert-info').addClass('alert-danger');
						} else {
							$memberInfo.removeClass('alert-danger').addClass('alert-info');
						}
						// ==============================================
					...
					$('#member_info').hide().removeClass('alert-danger').addClass('alert-info');
					$('#beneficiary_select').prop('disabled', true).html('<option value="">-- Select Beneficiary --</option>');
					$('#selected_member_id').val('');
				}
			}
	...
	// Function to load nominees (unchanged)
	...
	// Dynamic document upload fields (unchanged)
